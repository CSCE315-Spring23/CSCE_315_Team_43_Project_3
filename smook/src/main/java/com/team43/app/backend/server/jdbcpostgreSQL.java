...
import com.team43.app.backend.server.*; // NOTE: Important
import java.sql.*;
...
public class jdbcpostgreSQL {
    ...
    public ArrayList<String> getItemsInCategory(String category) {
        ...
        try {
            Statement stmt = conn.createStatement();
            String sqlStatement = "SELECT name FROM menu_item WHERE type=\'" + category + "\'";
            ResultSet result = stmt.executeQuery(sqlStatement);
    ...
    public ArrayList<String> getAllIngredients() {
        ...
        try {
            Statement stmt = conn.createStatement();
            String sqlStatement = "SELECT name FROM menu_item WHERE name NOT IN ('Small Cup', 'Medium Cup', 'Large Cup', 'Small Straw', 'Large Straw', 'Assorted Snacks', 'Dummy Item')";
            ResultSet result = stmt.executeQuery(sqlStatement);
    ...
    public MenuItem getMenuItem(String menu_item) {
        ...
        try {
            Statement stmt = conn.createStatement();
            String sqlStatement = "SELECT * FROM menu_item WHERE name=\'" + menu_item + "\'";
            ResultSet result = stmt.executeQuery(sqlStatement);
            ...
            sqlStatement = "SELECT * FROM ingredient_list WHERE menu_id=" + item.getID();
    ...
    public void writeTransactionData(Transaction trans, int emp_id) {
        try {
            Statement stmt = conn.createStatement();
            String sqlStatement = "INSERT INTO transaction (transaction_id, employee_id, purchaser_name, price_of_transaction, time_of_purchase) VALUES ("
                    +
            ...
            for (Integer menu_id : trans.getMenuItemIDs()) {
                sqlStatement = "INSERT INTO transaction_item (menu_id, transaction_id) VALUES (" +
                        menu_id + ", " + trans.getID() + ")";
                stmt.executeUpdate(sqlStatement);

                // NOTE: Build single insert then execute instead?
            }
        } catch (Exception e) {
            e.printStackTrace();
            System.err.println(e.getClass().getName() + ": " + e.getMessage());
