...
public class jdbcpostgreSQL {
  ...
  public void addInventoryItem(String name, double price, double quantity,
    ...
    try {
      ...
      Statement stmt = conn.createStatement();
      String sqlStatement =
          "INSERT INTO inventory (inventory_id, name, price, quantity, measurement_type) VALUES (" +
          next_index + ", \'" + name + "\', " + price + ", " + quantity +
          ", \'" + measurement_type + "\');";
  ...
  public List<List<Integer>> findPairs(String start, String end) {
    ...
    try {
      ...
      // Running a query
      String sqlStatement =
          "SELECT O1.menu_id AS menu_id_1, O2.menu_id AS menu_id_2, COUNT(*) AS PurchaseFrequency FROM transaction_item AS O1 INNER JOIN transaction_item AS O2 ON O1.transaction_id = O2.transaction_id AND O1.menu_id < O2.menu_id AND O2.menu_id < 68 WHERE O1.transaction_id IN (SELECT transaction_id FROM transaction WHERE transaction.time_of_purchase BETWEEN CAST('" +
          start + "' AS DATE) AND CAST('" + end +
          "' AS DATE)) AND O2.transaction_id IN (SELECT transaction_id FROM transaction WHERE transaction.time_of_purchase BETWEEN CAST('2022-05-01' AS DATE) AND CAST('2022-10-05' AS DATE)) GROUP BY menu_id_1, menu_id_2 ORDER BY PurchaseFrequency DESC;";
  ...
  public List<List<String>> getSalesReport(String startDate, String endDate) {
    try {
      ...
      String sqlStatement =
          "SELECT * from transaction WHERE time_of_purchase between '" +
      ...
      String newTable =
          "CREATE TABLE sales_report AS SELECT menu_id, name, type, price, ingredient_amount, 0 AS amount_ordered FROM menu_item;";
      stmt.executeUpdate(newTable);

      // Update the amount_ordered value in the sales report table
      String getTransactionItems =
          "SELECT * FROM transaction_item WHERE transaction_id BETWEEN " +
          lowestID + " AND " + highestID + ";";
      ...
      while (getTable.next()) {
        ...
        if (hm.containsKey(Integer.parseInt(getTable.getString("menu_id"))))
          hm.put(Integer.parseInt(getTable.getString("menu_id")),
                 hm.get(Integer.parseInt(getTable.getString("menu_id"))) + 1);
      ...
      for (Map.Entry<Integer, Integer> mapElement : hm.entrySet()) {
        ...
        String increment = "UPDATE sales_report SET amount_ordered = " +
                           mapElement.getValue() + " WHERE menu_id = " + key +
                           ";";
  ...
  public HashMap<String, ArrayList<Float>> getRestock() {
    ...
    for (Integer id : curr_inventory.keySet()) {
      if (avg_usage.containsKey(id) &&
          curr_inventory.get(id) < (7 * avg_usage.get(id))) {
        ArrayList<Float> value = new ArrayList<Float>();
        value.add(curr_inventory.get(id));
        value.add(7 * avg_usage.get(id));
